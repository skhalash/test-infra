...
function generateLetsEncryptCert() {
    ...
    gsutil cp "./letsencrypt/live/${DOMAIN}/${DOMAIN}.cert.encrypted" "${KYMA_SECRETS_BUCKET}/certificates/"
    gsutil cp "./letsencrypt/live/${DOMAIN}/${DOMAIN}.key.encrypted" "${KYMA_SECRETS_BUCKET}/certificates/"    
...
gsutil -q stat "${KYMA_SECRETS_BUCKET}/certificates/${DOMAIN}.cert.encrypted"
VALID_CERT_FILE=$?
gsutil -q stat "${KYMA_SECRETS_BUCKET}/certificates/${DOMAIN}.key.encrypted"
VALID_KEY_FILE=$?
...
if [[ $VALID_CERT_FILE -eq 0 && $VALID_KEY_FILE -eq 0 ]]; then
    ...
    gsutil cp "${KYMA_SECRETS_BUCKET}/certificates/${DOMAIN}.cert.encrypted" "./letsencrypt/live/${DOMAIN}" 
    gsutil cp "${KYMA_SECRETS_BUCKET}/certificates/${DOMAIN}.key.encrypted" "./letsencrypt/live/${DOMAIN}" 
